function all_fits_lsqcurvefit_MultiStart_SLS_1_only_set_tau(data,name,i)
    ...
    if contains(name,'20k')
        funct_MW=@SLS_relax_Funct_General_1_term_Set_Tau_20k_12h;
    elseif contains(name,'60k')
        funct_MW=@SLS_relax_Funct_General_1_term_Set_Tau_60k_12h;
    elseif contains(name,'100k')
        funct_MW=@SLS_relax_Funct_General_1_term_Set_Tau_100k_12h;
    elseif contains(name,'40k') %FEquivalent from free fit
        funct_MW=@SLS_relax_Funct_General_1_term_Set_Tau_40k_12h;
    elseif contains(name,'500k')
        funct_MW=@SLS_relax_Funct_General_1_term_Set_Tau_500k_12h;
    elseif contains(name,'1000k')
        funct_MW=@SLS_relax_Funct_General_1_term_Set_Tau_1000k_12h;
    end


    % Set initial guess for the model parameters
    p_init_SLS_1 = [1 1];  %(A, B), SLS 1-term
    ...
    'lb',[0, 0],'ub',[],'xdata',Step_time,'ydata',Normalized_Stress_G);
   ...
   %Saving the calculated x and y values in a table to plot on Prism
    ...
    filename = append(name," - All Fits - SLS 1 Only.xls");
    ...
    filename_coeff = append(name," - Coefficients SLS 1 only.xls");
